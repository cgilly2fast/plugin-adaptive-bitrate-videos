...
describe('Plugin tests', () => {
    ...
    it('standard video outputs are present', () => {
        ...
        for (let i = 0; i < resolutions.length; i++) {
            for (let j = 0; j < exceptedNumSegments; j++) {
                expect(
                    fs.existsSync(
                        `src/override-segments/testVideo-${resolutions[i]}p-segment${j}.ts`,
                    ),
                ).toBe(true)
            }
        }
    ...
    it('custom video outputs are present', () => {
        ...
        for (let i = 0; i < resolutions.length; i++) {
            for (let j = 0; j < exceptedNumSegments; j++) {
                expect(
                    fs.existsSync(
                        `./src/override-segments/testVideo2-${resolutions[i]}p-segment${j}.ts`,
                    ),
                ).toBe(true)
            }
        }
    ...
    it('custom resolution manifest playlists are present', () => {
        ...
        for (let i = 0; i < resolutions.length; i++) {
            expect(
                fs.existsSync(
                    `./src/override-segments/testVideo2-${resolutions[i]}p-playlist.m3u8`,
                ),
            ).toBe(true)
        }
    })
